...
var myContentCallBack = function(data){
  console.log("Got repository content");
  this.Sites = data;
  $(".loader").hide();
...
function renderRepositoryContent(){
  ...
  for(var i = 0; i < RepoResources.length; i++){
    if($("#"+RepoResources[i].label).length == 0)
      table.append("<tr id='"+RepoResources[i].label+"' class='repo-course-folder' name='"+i+"'><td ><i class='material-icons' >arrow_right folder</i></td><td class='folder'><span>"+RepoResources[i].label+"</span></td><td>"+RepoResources[i].ownerId+"</td></tr>");
  }

  $(".repo-course-folder").click(function(){
      console.log("Index : "+$(this).attr("name"));
      renderCourseDataStreams($(this).attr("name"));
  });
}
function renderCourseDataStreams(index){
  $(".yearClass").show();
  $(".yearClass").first().html(RepoResources[index].label);
  table = $("#RepoResourceTable");
  table.html("");

  table.append("<thead class='Repo-Assignments' ></thead>");
  var repo_assignments_classname = ".Repo-Assignments";
  $(repo_assignments_classname).append('<tr class="folder"><td ><i class="material-icons" name="'+repo_assignments_classname+'">arrow_drop_down folder_open</i></td><td >Assignments</td><td></td><td></td></tr>');

  table.append("<thead class='Repo-Notes' ></thead>");
  var repo_notes_classname = ".Repo-Notes";
  $(repo_notes_classname).append('<tr class="folder"><td ><i class="material-icons" name="'+repo_notes_classname+'">arrow_drop_down folder_open</i></td><td >Notes and Miscellaneous</td><td></td><td></td></tr>');

  table.append("<thead class='Repo-Exams' ></thead>");
  var repo_exams_classname = ".Repo-Exams";
  $(repo_exams_classname).append('<tr class="folder"><td ><i class="material-icons" name="'+repo_exams_classname+'">arrow_drop_down folder_open</i></td><td >Assignments</td><td></td><td></td></tr>');

  var DataStreams = RepoResources[index].datastreams[0].objectDatastreams.datastream;
  for(var i = 0;  i < DataStreams.length; i++){
      $(repo_assignments_classname).append("<tr class='repo-year-folder' name='"+index+","+i+"'><td ><i class='material-icons' >insert_drive_file</i></td><td class='folder'><span>"+DataStreams[i].dsid+"</span></td><td><a href='"+getURL(RepoResources[index].datastreams[0].objectDatastreams.pid,DataStreams[i].dsid)+"'  target='_blank'>"+DataStreams[i].label+"</a></td><td>"+DataStreams[i].mimeType+"</td></tr>");
  }
}
